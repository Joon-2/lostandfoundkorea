...
function buildEmail({ name, caseNumber }) {
    ...
    <tr><td align="center">
      <table role="presentation" width="560" cellpadding="0" cellspacing="0" style="max-width:560px;background:#ffffff;border:1px solid #e2e8f0;border-radius:16px;overflow:hidden;">
        ...
        <tr><td style="padding:32px 28px;">
          <p style="margin:0 0 16px;font-size:16px;color:#1a202c;">${greeting}</p>
          ...
          <div style="margin:24px 0;padding:16px 20px;background:#f8fafb;border:1px solid #e2e8f0;border-radius:12px;">
            ...
            <div style="margin-top:4px;font-family:'SFMono-Regular',Menlo,monospace;font-size:22px;font-weight:600;color:#1a202c;letter-spacing:0.06em;">${caseNumber}</div>
...
export async function sendConfirmationEmail({ name, email, caseNumber }) {
  ...
  const { subject, text, html } = buildEmail({ name, caseNumber });
...
function buildPaymentEmail({ name, caseNumber }) {
  const greeting = name ? `Hi ${name},` : "Hi there,";
  const subject = `Good news — we have a lead on your item (${caseNumber})`;
...
To unlock the exact recovery location, contact info, and step-by-step English pickup instructions, complete the $39 recovery payment:
${link}
    ...
    <tr><td align="center">
      <table role="presentation" width="560" cellpadding="0" cellspacing="0" style="max-width:560px;background:#ffffff;border:1px solid #e2e8f0;border-radius:16px;overflow:hidden;">
        ...
        <tr><td style="padding:32px 28px;">
          <p style="margin:0 0 16px;font-size:16px;color:#1a202c;">${greeting}</p>
          <p style="margin:0 0 16px;font-size:16px;line-height:1.55;color:#4a5568;">
            Great news &mdash; we have a strong lead on your lost item (case <strong style="color:#1a202c;">${caseNumber}</strong>).
          </p>
          <p style="margin:0 0 16px;font-size:15px;line-height:1.55;color:#4a5568;">
            Complete the <strong style="color:#1a202c;">$39 recovery payment</strong> to unlock the exact location, contact info, and step-by-step English pickup instructions.
          </p>
          <p style="margin:24px 0;">
            <a href="${link}" style="display:inline-block;background:#059669;color:#ffffff;padding:14px 28px;border-radius:9999px;text-decoration:none;font-weight:600;font-size:15px;">Unlock recovery details</a>
          </p>
