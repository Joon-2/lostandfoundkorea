...
Thanks for submitting your lost item report to ${SITE_NAME}. Our team has received your case and will start searching right away.
...
Need urgent help? Chat with us on WhatsApp: ${WA_URL}
      ...
      <table role="presentation" width="560" cellpadding="0" cellspacing="0" style="max-width:560px;background:#ffffff;border:1px solid #e2e8f0;border-radius:16px;overflow:hidden;">
        ...
        <tr><td style="padding:32px 28px;">
          ...
          <p style="margin:0 0 8px;font-size:15px;line-height:1.55;color:#4a5568;">
            If we locate your item, you can unlock the full recovery details for <strong style="color:#1a202c;">$${plans.recovery.paymentPrice}</strong>.
          </p>
          ...
          <p style="margin:0;">
            <a href="${WA_URL}" style="display:inline-block;background:#25D366;color:#ffffff;padding:12px 22px;border-radius:9999px;text-decoration:none;font-weight:500;font-size:14px;">Chat with us on WhatsApp</a>
          </p>
        </td></tr>
        <tr><td style="padding:18px 28px;background:#f8fafb;border-top:1px solid #e2e8f0;font-size:12px;color:#718096;text-align:center;">
          ${escapeHtml(SITE_NAME)} &middot; ${escapeHtml(SITE_LOCATION)} &middot; <a href="${SITE_URL}" style="color:#718096;text-decoration:underline;">${escapeHtml(SITE_HOSTNAME)}</a>
        </td></tr>
      </table>
    </td></tr>
  </table>
...
function buildPaymentEmail({
  ...
  const link = `${SITE_URL}/pay/${caseNumber}`;
...
If you have any new information that might help, reply to this email or chat with us on WhatsApp: ${WA_URL}
  ...
  <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="background:#f8fafb;padding:32px 16px;">
    <tr><td align="center">
      <table role="presentation" width="560" cellpadding="0" cellspacing="0" style="max-width:560px;background:#ffffff;border:1px solid #e2e8f0;border-radius:16px;overflow:hidden;">
        ...
        <tr><td style="padding:18px 28px;background:#f8fafb;border-top:1px solid #e2e8f0;font-size:12px;color:#718096;text-align:center;">
          ${escapeHtml(SITE_NAME)} &middot; <a href="${SITE_URL}" style="color:#718096;text-decoration:underline;">${escapeHtml(SITE_HOSTNAME)}</a>
        </td></tr>
      </table>
    </td></tr>
  </table>
...
Recovery details are available at ${SITE_URL}/pay/${caseNumber}
      ...
      <table role="presentation" width="560" cellpadding="0" cellspacing="0" style="max-width:560px;background:#ffffff;border:1px solid #e2e8f0;border-radius:16px;overflow:hidden;">
        ...
        <tr><td style="padding:32px 28px;">
          ...
          <p style="margin:16px 0 0;">
            <a href="${SITE_URL}/pay/${encodeURIComponent(caseNumber)}" style="display:inline-block;background:#059669;color:#ffffff;padding:12px 22px;border-radius:9999px;text-decoration:none;font-weight:600;font-size:14px;">Open case page</a>
          </p>
        </td></tr>
        <tr><td style="padding:18px 28px;background:#f8fafb;border-top:1px solid #e2e8f0;font-size:12px;color:#718096;text-align:center;">
          ${escapeHtml(SITE_NAME)} &middot; ${escapeHtml(SITE_LOCATION)} &middot; <a href="${SITE_URL}" style="color:#718096;text-decoration:underline;">${escapeHtml(SITE_HOSTNAME)}</a>
...
function buildShippingQuoteEmail({ name, caseNumber, amount, notes, shippingAddress }) {
  const greeting = name ? `Hi ${name},` : "Hi there,";
...
We've prepared a shipping quote to deliver your recovered item.

Amount: $${amount}
${addressBlock}${notesBlock}
Reply to this email to approve the quote, or chat with us on WhatsApp: ${WA_URL}
    ...
    <tr><td align="center">
      <table role="presentation" width="560" cellpadding="0" cellspacing="0" style="max-width:560px;background:#ffffff;border:1px solid #e2e8f0;border-radius:16px;overflow:hidden;">
        ...
        <tr><td style="padding:32px 28px;">
          ...
          <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="margin:20px 0;border:1px solid #e5e7eb;border-radius:12px;overflow:hidden;">
            <tr><td style="padding:12px 16px;font-size:13px;color:#718096;background:#f8fafb;border-bottom:1px solid #e5e7eb;">Shipping cost</td>
                <td style="padding:12px 16px;font-size:16px;color:#1a202c;background:#f8fafb;border-bottom:1px solid #e5e7eb;font-weight:600;text-align:right;">$${escapeHtml(String(amount))}</td></tr>
            ${
              shippingAddress
                ? `<tr><td style="padding:12px 16px;font-size:13px;color:#718096;vertical-align:top;">Shipping to</td>
                     <td style="padding:12px 16px;font-size:14px;color:#1a202c;text-align:right;white-space:pre-wrap;">${escapeHtml(shippingAddress)}</td></tr>`
                : ""
            }
            ${
              notes
                ? `<tr><td style="padding:12px 16px;font-size:13px;color:#718096;vertical-align:top;background:#f8fafb;">Details</td>
                     <td style="padding:12px 16px;font-size:14px;color:#1a202c;background:#f8fafb;text-align:right;white-space:pre-wrap;">${escapeHtml(notes)}</td></tr>`
                : ""
            }
            <tr><td style="padding:12px 16px;font-size:13px;color:#718096;">Case reference</td>
                <td style="padding:12px 16px;font-size:14px;color:#1a202c;text-align:right;font-family:'SFMono-Regular',Menlo,monospace;">${escapeHtml(caseNumber)}</td></tr>
          </table>
          <p style="margin:16px 0;font-size:14px;line-height:1.55;color:#4a5568;">
            Reply to this email to approve the quote, or chat with us on WhatsApp.
          </p>
          <p style="margin:16px 0;">
            <a href="${WA_URL}" style="display:inline-block;background:#25D366;color:#ffffff;padding:12px 22px;border-radius:9999px;text-decoration:none;font-weight:500;font-size:14px;">Chat on WhatsApp</a>
          </p>
        </td></tr>
        <tr><td style="padding:18px 28px;background:#f8fafb;border-top:1px solid #e2e8f0;font-size:12px;color:#718096;text-align:center;">
          ${escapeHtml(SITE_NAME)} &middot; <a href="${SITE_URL}" style="color:#718096;text-decoration:underline;">${escapeHtml(SITE_HOSTNAME)}</a>
...
export async function sendShippingQuoteEmail({
  ...
  const { subject, text, html } = buildShippingQuoteEmail({
    name,
    caseNumber,
    amount,
    notes,
...
Your item is on its way. Here are the tracking details:

Tracking number: ${trackingNumber || "—"}
Shipping method: ${shippingMethod || "—"}
${dateStr ? `Estimated delivery: ${dateStr}\n` : ""}
Case reference: ${caseNumber}
    ...
    <tr><td align="center">
      <table role="presentation" width="560" cellpadding="0" cellspacing="0" style="max-width:560px;background:#ffffff;border:1px solid #e2e8f0;border-radius:16px;overflow:hidden;">
        ...
        <tr><td style="padding:32px 28px;">
          ...
          <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="margin:20px 0;border:1px solid #e5e7eb;border-radius:12px;overflow:hidden;">
            <tr><td style="padding:12px 16px;font-size:13px;color:#718096;background:#f8fafb;border-bottom:1px solid #e5e7eb;">Tracking number</td>
                <td style="padding:12px 16px;font-size:14px;color:#1a202c;background:#f8fafb;border-bottom:1px solid #e5e7eb;font-family:'SFMono-Regular',Menlo,monospace;text-align:right;word-break:break-all;">${escapeHtml(trackingNumber || "—")}</td></tr>
            <tr><td style="padding:12px 16px;font-size:13px;color:#718096;border-bottom:1px solid #e5e7eb;">Shipping method</td>
                <td style="padding:12px 16px;font-size:14px;color:#1a202c;border-bottom:1px solid #e5e7eb;text-align:right;">${escapeHtml(shippingMethod || "—")}</td></tr>
            ${
              dateStr
                ? `<tr><td style="padding:12px 16px;font-size:13px;color:#718096;background:#f8fafb;border-bottom:1px solid #e5e7eb;">Estimated delivery</td>
                     <td style="padding:12px 16px;font-size:14px;color:#1a202c;background:#f8fafb;border-bottom:1px solid #e5e7eb;text-align:right;">${escapeHtml(dateStr)}</td></tr>`
                : ""
            }
            <tr><td style="padding:12px 16px;font-size:13px;color:#718096;">Case reference</td>
                <td style="padding:12px 16px;font-size:14px;color:#1a202c;text-align:right;font-family:'SFMono-Regular',Menlo,monospace;">${escapeHtml(caseNumber)}</td></tr>
