...
// Layout: header banner → greeting → body → footer with reply-by-email
// reminder + small WhatsApp link.
function renderEmail({ greeting, bodyHtml, bodyText }) {
  ...
  <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="background:#f8fafb;padding:32px 16px;">
    <tr><td align="center">
      <table role="presentation" width="560" cellpadding="0" cellspacing="0" style="max-width:560px;background:#ffffff;border:1px solid #e2e8f0;border-radius:16px;overflow:hidden;">
        ...
        <tr><td style="padding:20px 28px;background:#f8fafb;border-top:1px solid #e2e8f0;font-size:13px;color:#4a5568;text-align:center;">
...
No item found? You pay nothing.`;
  ...
  const { html, text } = renderEmail({ greeting, bodyHtml, bodyText });
...
function buildInfoRequestEmail({ name, caseNumber, infoText }) {
  ...
  const bodyHtml = [
    ...
    `<p style="margin:0 0 14px;color:#1a202c;">${escapedInfo}</p>`,
    strongLine("Case reference", caseNumber),
  ].join("");

  const bodyText = `Thanks for your report. To help us find your item faster, could you provide:
